...
function GM:PlayerRequestClass( ply, class, disablemessage )
	...
	if ( ply:Alive() && SERVER ) then
		...
		if ( !disablemessage ) then
			ply:ChatPrint( "Ваш класс будет сменён на '".. player_class.GetClassName( RequestedClass ) .. "' после возрождения" )
		end
...
function GM:OnPlayerChangedTeam( ply, oldteam, newteam )
	...
	//PrintMessage( HUD_PRINTTALK, Format( "%s зашел за '%s'", ply:Nick(), team.GetName( newteam ) ) )
		...
		if ( id ~= highest and id > 0 && id < 1000 && team.Joinable( id ) ) then
			if team.NumPlayers( id ) < team.NumPlayers( highest ) then
				while team.NumPlayers( id ) < team.NumPlayers( highest ) - 1 do
					...
					PrintMessage(HUD_PRINTTALK, ply:Name().." был перенесён к "..team.GetName( id ).." из-за баланса. ("..reason..")" )
...
function GM:EndOfGame( bGamemodeVote )
	...
	if ( bGamemodeVote ) then
		...
		MsgN( "Голосование..." )
		PrintMessage( HUD_PRINTTALK, "Начинаем голосование..." );

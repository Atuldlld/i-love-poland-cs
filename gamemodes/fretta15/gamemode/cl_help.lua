...
function GM:ShowHelp()
	...
	if ( !IsValid( Help ) ) then
		...
		Help.lblFooterText.Think = function( panel )
										...
										if( GetGlobalBool( "IsEndOfGame", false ) ) then panel:SetText( "Битва закончилась..." ) return end
										if( GAMEMODE.RoundBased && CurTime() > GAMEMODE:GetTimeLimit() ) then panel:SetText( "Игра завершится после этой битвы..." ) return end

										panel:SetText( "Время: " .. util.ToMinutesSeconds( tl ) )
									end

		-- if ( GetConVarNumber( "fretta_voting" ) != 0 ) then
		-- 	local btn = Help:AddSelectButton( "Голосование за смены", function() RunConsoleCommand( "voteforchange" ) end )
		-- 	btn.m_colBackground = Color( 255, 200, 100 )
		-- 	btn:SetDisabled( LocalPlayer():GetNWBool( "WantsVote" ) )
		-- end  --смена режима

		if ( GAMEMODE.TeamBased ) then
			local btn = Help:AddSelectButton( "Сменить команду", function() GAMEMODE:ShowTeam() end )
		...
		if ( !GAMEMODE.TeamBased && GAMEMODE.AllowSpectating ) then
			...
			if ( LocalPlayer():Team() == TEAM_SPECTATOR ) then
				...
				local btn = Help:AddSelectButton( "Зайти в игру", function() RunConsoleCommand( "changeteam", TEAM_UNASSIGNED ) end )
				btn.m_colBackground = Color( 120, 255, 100 )

			else

				local btn = Help:AddSelectButton( "Наблюдать", function() RunConsoleCommand( "changeteam", TEAM_SPECTATOR ) end )
		...
		if ( IsValid( LocalPlayer() ) ) then
			...
			if ( Classes && #Classes > 1 ) then
				local btn = Help:AddSelectButton( "Сменить класс", function() GAMEMODE:ShowClassChooser( LocalPlayer():Team() ) end )
				btn.m_colBackground = Color( 120, 255, 100 )
			end
		...
		if ( GAMEMODE.SelectModel ) then
			...
			Help:AddPanelButton( "icon16/user.png", "Выбрать модель", CreateModelPanel )
		...
		if ( GAMEMODE.SelectColor ) then
			...
			Help:AddPanelButton( "icon16/application_view_tile.png", "Выбрать цвет игрока", CreateColorPanel )
